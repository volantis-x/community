...
{% note info, 为了兼容老用户，旧的 3.0 标签插件在重构之前依然沿用旧的格式，即 **command** + **空格** 作为参数分隔符，而部分 4.0 新增标签插件是 **空格** + **英文竖线** + **空格** ，请注意区分。 %}

{% note info, 我们也在探索哪种分隔符既简单又不容易引起冲突，所以 4.0 可能存在多种格式，具体以对应文档描述为准。 %}

{% note info :: 5.0 版本起临时实验使用英文双冒号 ( \:\: ) 作为分隔符分隔参数和文本，之前版本的分隔符写法向下兼容 %}

{% link 新版标签插件::/v6/tag-plugins/::https://unpkg.com/volantis-static@0.0.1649552113628/media/twemoji/assets/svg/1f433.svg %}
